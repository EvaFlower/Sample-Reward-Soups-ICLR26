...
class CustomDataset(Dataset):
    def __init__(self, meta_json_path, pretrained_tokenzier_path):
        if meta_json_path == 'coco':
            ...
            self.meta = prompts
        elif meta_json_path == 'drawbench':
            dataset = load_dataset("shunk031/DrawBench", cache_dir=dataset_cache_dir, split='test')
            prompts = [entry['prompts'] for entry in dataset][:50]
            self.meta = prompts
