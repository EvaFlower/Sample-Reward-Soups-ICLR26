        ...
        for i, t in enumerate(timesteps):
            for j in range(inner_steps):
                    ...
                    if  soup_start_step <= i < soup_end_step:    # i < ws_start_step
                        ...
                        for idx in anchor_ws_index:     
                            ...
                            for k in range(query_size//query_batch_size):
                                ...
                                elif weights[idx][1] == 1:
                                    func2_scores = compute_reward(
                                        prev_pred_x0, opt_func_name[1], pref_function[1],
                                        extra_info, query_batch_size, device
                                    )
                                    all_rewards.append(func2_scores)
                                elif weights[idx][2] == 1:
                                    func3_scores = compute_reward(
                                        prev_pred_x0, opt_func_name[2], pref_function[2],
                                        extra_info, query_batch_size, device
                                    )
                                    all_rewards.append(func3_scores)
                                else:  #for w!=0 or w!=1
                            ...
                            if weights[idx][0] == 1 or weights[idx][1] == 1 or weights[idx][2] == 1:
                                all_rewards = torch.cat(all_rewards, dim=0)
                                all_rewards_nor = normalize_reward(all_rewards, reward_norm)
                        ...
                        for idx in remaining_ws_indx:
                            ...
                            if len(anchor_all_rewards) == 3:
                                w = weights[idx]
                                w_all_rewards = (w[0]*anchor_all_rewards[0]+w[1]*anchor_all_rewards[1]+w[2]*anchor_all_rewards[2]).reshape(-1, 1, 1, 1)
                            else:  # anchors are not w=0 and w=1
                                raise NotImplementedError
                        ...
                        for idx, w in enumerate(weights):     
                            ...
                            for k in range(query_size//query_batch_size):
                                ...
                                all_rewards_3.append(func3_scores)
                            if isinstance(all_prev_latents_noise, list):
                                all_prev_latents_noise = torch.stack(all_prev_latents_noise, dim=0)
                            all_rewards_1 = torch.cat(all_rewards_1, dim=0)
                            all_rewards_2 = torch.cat(all_rewards_2, dim=0)
                            all_rewards_3 = torch.cat(all_rewards_3, dim=0)

                            all_rewards_1_nor = normalize_reward(all_rewards_1, reward_norm)
                            all_rewards_2_nor = normalize_reward(all_rewards_2, reward_norm)
                            all_rewards_3_nor = normalize_reward(all_rewards_3, reward_norm)

                            w_all_rewards = (w[0]*all_rewards_1_nor+w[1]*all_rewards_2_nor+w[2]*all_rewards_3_nor).reshape(-1, 1, 1, 1)
                            prev_noise_mean = torch.sum(w_all_rewards * all_prev_latents_noise, dim=0, keepdim=True)
                            latent_dim = np.prod(latents.shape[1:])
                            prev_noise_mean = prev_noise_mean/(prev_noise_mean ** 2).sum(axis=[1, 2, 3], keepdim=True).sqrt()*(latent_dim**0.5)
                            prev_sample = prev_latents_mean + prev_latents_std * prev_noise_mean
                ...
                if i == len(timesteps) - 1:
                    ...
                    # preference_scores = get_function_scores(opt_func_name[0], pref_function[0], prev_pred_x0, len(weights), \
                    #     pred_dict, accelerator, extra_info)
                    preference_scores = compute_reward(
                        prev_pred_x0, opt_func_name[0], pref_function[0],
                        extra_info, len(weights), device
                    )
                    images = pred_x0
